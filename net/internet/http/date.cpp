...
time_t net::internet::http::date::parse(const void* buf, size_t len, struct tm& timestamp)
{
	// RFC 2616
	...
	size_t n = ptr - (const unsigned char*) buf;
...
time_t net::internet::http::date::parse_ansic(const unsigned char* begin, const unsigned char* end, struct tm& timestamp)
{
...
bool net::internet::http::date::parse_month(const unsigned char* ptr, unsigned& mon)
{
	switch (*ptr) {
		case 'J':
		case 'j':
...
bool net::internet::http::date::parse_time(const unsigned char* ptr, unsigned& hour, unsigned& min, unsigned& sec)
{
	// Parse hour.
	if ((!IS_DIGIT(*ptr)) || (!IS_DIGIT(*(ptr + 1)))) {
		return false;
